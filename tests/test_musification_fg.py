"""Unit tests for musification_fg.py.

Pure-logic functions (no NIST, no MIDI rendering) are tested with synthetic
inputs. The integration entry point `molecular_music_fg` is tested with a
mock compound and a synthetic spectrum, then the produced MIDI file is removed.
"""
...
# Make src/chemecho importable without requiring `pip install -e .`
...
class TestFindAbsorptionPeaks:
    ...
    def test_slot_indexes_music_timeline(self):
        # slot = original_index - 1 (because peak_detection skips index 0 and N-1)
        wn, trans = make_synthetic_spectrum(peak_cm1=[1700], depths=[60])
        peaks = find_absorption_peaks(wn, trans)
        for p in peaks:
